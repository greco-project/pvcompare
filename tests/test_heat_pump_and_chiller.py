...
class TestCalculateCopsAndEers:
    ...
    def setup_class(self):
        ...
        self.weather = pd.DataFrame([11.85, 6.85, 2.0, 0.0, -3.0, 27.0], columns=["temp_air"],
    ...
    def test_calculate_cops_and_eers_heat_pump_without_icing(self):
        ...
        cops_exp = pd.Series([4.0, 3.0, 2.4144869215291727, 2.234636871508378, 2.01005025125628, np.nan],
    ...
    def test_calculate_cops_and_eers_heat_pump_consider_icing(self, add_icing_to_csv):
        cops = hc.calculate_cops_and_eers(
            ...
            input_directory=TEST_DATA_HEAT,
            mvs_input_directory=TEST_DATA_HEAT)
        cops_exp = pd.Series([4.0, 3.0, 2.4144869215291727, 1.7877094972067027, 1.608040201005024, np.nan],
                             index=self.date_range, name='no_unit')
        assert_series_equal(cops, cops_exp)
...
class TestAddSectorCoupling:
    ...
    def setup_class(self):
        ...
        self.lat = 53.2
        self.lon = 13.2
        self.filename_conversion = os.path.join(TEST_DATA_HEAT, "csv_elements", "energyConversion.csv")
    ...
    def test_add_sector_coupling_heat_pump_file_already_exists(self, select_conv_tech):
        ...
        hc.add_sector_coupling(weather=self.weather, lat=self.lat, lon=self.lon,
                               input_directory=TEST_DATA_HEAT,
                               mvs_input_directory=TEST_DATA_HEAT)
        # no file created
        filename = os.path.join(TEST_DATA_HEAT, "time_series", "cops_heat_pump_2018_53.2_13.2.csv")
    ...
    def test_add_sector_coupling_heat_pump_file_created(self, select_conv_tech):
        ...
        hc.add_sector_coupling(weather=self.weather, lat=self.lat, lon=self.lon,
                               ...
                               mvs_input_directory=TEST_DATA_HEAT)
        # file created
        filename = os.path.join(TEST_DATA_HEAT, "time_series",
                                "cops_heat_pump_2018_53.2_13.2.csv")
        assert os.path.exists(filename) == True
        # filename in energyConversion.csv changed
        df = pd.read_csv(self.filename_conversion, header=0, index_col=0)
        assert ("cops_heat_pump_2018_53.2_13.2.csv" in df.loc["efficiency"].heat_pump_file_non_existent) == True
