...
class TestPlotProfiles:
    ...
    def test_plot_all_flows(self):
        ...
        filename = os.path.join(self.output_directory, f"plot_{timeseries_name}{period}.png")
    ...
    def test_plot_kpi_loop(self):
        ...
        filename = os.path.join(self.output_directory, "plot_scalars_" + str(variable_name) + ".png")
        if os.path.exists(filename):
            os.remove(filename)

        plot_kpi_loop(variable_name = variable_name, kpi = [
            "costs total PV",
            "Degree of autonomy"],
