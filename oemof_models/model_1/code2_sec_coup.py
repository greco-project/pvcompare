"""
This model includes sector coupling with the heat sector included. The sector coupling is achieved through the
addition of a heat pump. In addition, there is also a thermal storage.
...
from oemof.solph import (Sink, Source, Bus, Flow, EnergySystem, Investment, Model, Transformer)
...
demand_th = Sink(label='demand_th', inputs={thbus: Flow(nominal_value=1, actual_value=data['demand_th'], fixed=True)})
excess_el = Sink(label='excess_el', inputs={elbus: Flow(variable_costs=-0.005)})
shortage_el = Source(label='shortage_el', outputs={elbus: Flow(variable_costs=1e10)})
...
es.add(excess_el, demand_el, el_storage, th_storage, pv, shortage_el, elbus, thbus)
